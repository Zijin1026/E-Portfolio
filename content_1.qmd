---
title: "Project 1: Evaluating and Comparing the Performance of VRI Data and LiDAR-derived Stand Metrics across Different Geospatial Features in MKRF, BC"
format:
  html:
    theme: superhero
editor: visual
...
The Vegetation Resource Inventory (VRI) is a provincial database that records forest metrics across BC (BC Government., n.d.). In the modern context, VRI is no longer an ideal database for stand-level studies. The polygons contain limited information and are prone to overgeneralizing the metrics for the area within a polygon, which is likely due to and introduced by the aggregation effects (Holt et, al., 2010). However, VRI still actively participates in a lot of fields like estimating carbon stock, disturbance map updating, and various other fields today (Parshakov et. al., 2025; DellaSala et. al., 2022).
